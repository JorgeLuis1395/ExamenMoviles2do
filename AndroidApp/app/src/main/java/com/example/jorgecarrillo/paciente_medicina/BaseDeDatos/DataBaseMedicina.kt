package com.example.jorgecarrillo.paciente_medicina.BaseDeDatos
...
class DataBaseMedicina {
    ...
    companion object {
        ...
        fun getAplicacionesBusqueda(nombre:String): ArrayList<Medicina> {
            ...
            val policy = StrictMode.ThreadPolicy.Builder().permitAll().build()
            StrictMode.setThreadPolicy(policy)
            val (request, response, result) = "http://192.168.1.7:1337/Medicina/$nombre".httpGet().responseString()
            val jsonStringLibro = result.get()
        ...
        fun putAplicacionEstado(aplicacion: Medicina, estado: Int) {
            "\"http://192.168.1.7:1337/Medicina?pacienteId=0\"".httpPut(listOf("estado" to estado))
        ...
        fun getAplicacioneUnica(id: Int): Medicina {
            val policy = StrictMode.ThreadPolicy.Builder().permitAll().build()
            StrictMode.setThreadPolicy(policy)
            val (request, response, result) = "http://192.168.1.7:1337/Medicina/$id".httpGet().responseString()
            val jsonStringLibro = result.get()
            val parser = Parser()
            val stringBuilder = StringBuilder(jsonStringLibro)
